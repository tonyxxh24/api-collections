...
│  │  ├─ app/
│  │  │  ├─ page.tsx       # / API 列表
│  │  │  ├─ api/[id]/page.tsx   # /api/[id] API 詳細 + 測試面板
│  │  │  └─ dashboard/page.tsx  # /dashboard 視覺化看板
│  │  └─ components/
│  └─ api/                 # Express 後端
│     └─ src/
...
新增 provider 時，請於 `apps/api/src/providers/{name}.ts` 實作固定介面：
...
1. 建立 `apps/api/src/providers/my-provider.ts`，實作 `ApiProvider`。
2. 在 `apps/api/src/providers/index.ts` 匯入並註冊到 `providers` map。
